...
class ch_cwd_to_this_file:
    def __init__(self, _code_file_path):#_code_file_path usually pass __file__
...
def beijing_datetime()->datetime.datetime:
    """
    print(f'current Beijing time is {beijing_time:%Y.%m.%d %H:%M:%S}')
    """
    # get local timezone
...
def map_string_to_int(string,MIN,MAX):
    """
    distribute as uniformly as possible in [MIN,MAX]. Tests show variance similar to random (but if string has many duplicates, many duplicate ints)
    """
    ...
    # print("sum", sum)
    ret=2**sum
    ret += sum #otherwise all multiples of 2
    ret=ret%(MAX-MIN)
    ret+=MIN
    return ret
if 0:
