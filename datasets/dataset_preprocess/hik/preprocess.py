"""
modified based on SAST/sast/data/multi_person_data.py
"""
...
if 1:
    ...
    sys.path.append(os.path.abspath(os.path.join(cur_dir, 'SAST')))
...
def _load_dataset(dataset: str, cfg):
    ...
    # n_frames=scene.poses3d.shape[0]
...
def preprocess_sequences(
    ...
    """
    train and eval both call it
        (the input is 'sub-seq', 
        output can be feed into model. 
        this process mainly contains: 
            norm; 
            persons -> primary, others; 
            pad; present -> primary_exists, others_exists
    ...
    print(f"{n_persons, n_frames, n_joints, n_dim=}")
    ...
    print(f"{frames=}")
    print(f"{object_frame=}")
    print(f"{frames[object_frame]=}")

    # objs_raw = kitchen.get_environment(global_frame)

    if  1 :
        subseq=persons # P,T,J=29,3
        kid:str=kitchen.dataset #A/B/C/D
        global_frame=global_frame
        present=present
        tu=(subseq,kid,present)
        return tu
    for ip in range(n_persons):
...
def create_objects(objs_norm_pointclouds, objs_labels):
    ...
    obj_feats = [
        np.concatenate([BASIS_POINT_SET.query(norm_obj), label])# concat BPS with label
...
class MultiPersonData(data.Dataset):
    ...
    def load_from_hik(
        ...
        with multiprocessing.get_context("spawn").Pool(cfg.loader.num_workers) as pool:
            ...
            print(f'{len(datasets[0][0])=}')
            print(f'{len(datasets[0][1])=}')
            print(f'{len(datasets[0][2])=}')
            print(f'{len(datasets[0][3])=}')
            ...
            tus = pool.starmap(map_func, data_iter, chunksize=100)
            print(f'{len(tus)=}')
        ...
        if 1:
            ...
            if 1:
                tus_dir= Path(  save_path  )
                seq_path =tus_dir/"tus.pkl"
                import pickle
                with open(seq_path, 'wb') as f:
                    assert len(tus)==1,len(tus)
                    tu=tus[0]
                    pickle.dump(tu, f)
            print(f"==>> seq_path: {seq_path}",flush=True)
            return
    ...
    def load_from_file(cls, save_path):
        ...
        for k in tqdm(keys):
            data[k] = np.load(f"{save_path}/{k}.npy", mmap_mode="r")
    ...
    def __getitem__(self, idx):
        ...
        if 1:#for k,v , copy v. 
            # otherwise torch warning (caused by r mode for np.load mmap_mode)
...
def create_dataset(save_name, cfg_path, datasets="ABCD"):
    ...
    for dataset in datasets:
        ...
        DIR_=Path(  './hik_preprocessed' )
        DIR_.mkdir(exist_ok=1)
        DIR_=DIR_  /  f'H{cfg.data.frames_in}F{cfg.data.frames_out}'
        DIR_.mkdir(exist_ok=1)
        save_path = f"{DIR_}/{save_name}_{dataset}"
        os.makedirs(save_path,exist_ok=1)
        data = MultiPersonData.load_from_hik(cfg, load_only=dataset,save_path=save_path)
