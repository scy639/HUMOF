# modified from: https://github.com/Raibows/DynamicBatchSampler
...
class DynamicBatchSampler(Sampler):
    ...
    def wrap_A(
        ...
        if 1:
            # dataset.id2O:list, not dic
            O2ids= {}
            for id_,O in enumerate(dataset.id2O):
                if O not in O2ids:
                    O2ids[O]=[]
                O2ids[O].append(id_)
            if 1:#print len of each v. sort by k
                keys=sorted(O2ids.keys())
                for k in keys:
                    v=O2ids[k]
                    print(f"{len(v)} data items contain {k} objs")
                del keys
            # print(f"{O2ids=}")
            set_O=set(dataset.id2O )
            print(f"{set_O=}")
            min_len=min(set_O)
            max_len=max(set_O)
            if max_batch_tokens < max_len:
                print(f"[warning] {max_batch_tokens=}<{max_len=} => max_batch_tokens=max_len")
                max_batch_tokens = max_len
            num_buckets= max_len-min_len+1
            print(f"{min_len=},{max_len=},{num_buckets=}")
        batch_sampler = DynamicBatchSampler(
            1, 0, 
            dataset.id2O, 
            num_buckets=num_buckets, 
            # u can adjust based on GPU memory
    ...
    def __init__(self, num_replicas, rank, length_dict, num_buckets=128, min_len=0, max_len=1024,
        ...
        self._batches = []  # orig is __batches, i modify to _batches for debug

    def __len__(self):
        return self.__per_gpu_batch_num

    def __iter__(self):
        for batch in self._batches[self.rank:len(self._batches):self.num_replicas]:
            yield batch

    def set_epoch(self, epoch: int):
        self.__epoch = epoch
        self._batches = self.__prepare_batches()
    ...
    def __prepare_batches(self):
        ...
        if self.shuffle:
            ...
            if len(self._batches) > 0: return self._batches
            indices = list(range(len(self.length_dict)))
        batches = []
        buckets = [[] for _ in range(self.num_buckets)]
        buckets_max_len = [self.max_len for _ in range(self.num_buckets)] # now the func is buckets_min_len
        for idx in indices:
            ...
            idx_bkt = idx_len - self.min_len
            # buckets_max_len[idx_bkt] = max(buckets_max_len[idx_bkt], idx_len) 
            buckets_max_len[idx_bkt] = min(buckets_max_len[idx_bkt], idx_len)
            ...
            if self.__is_full(tokens_in_all, buckets[idx_bkt]):
                ...
                buckets[idx_bkt] = []
                buckets_max_len[idx_bkt] = self.max_len
        ...
        for idx in leftover_indices:
            ...
            if self.__is_full(tokens_in_all, leftover_batch):
                batches_leftover.append(leftover_batch)
                leftover_batch = []
                leftover_max_len = 0
            leftover_batch.append(idx)
        batches+=batches_leftover
        if 1:
            print(f"{batches_leftover[:3]=}")
            batches_leftover_2= [  [ self.length_dict[id_] for id_ in batch] for batch in  batches_leftover  ]
            print(f"{batches_leftover_2=}")
        if 0:
            print(f"{leftover_indices=}")
            print(f"{leftover_batch=}")
            leftover_batch_2= [   self.length_dict[id_]   for id_ in  leftover_batch  ]
            print(f"{leftover_batch_2=}")
